...
class Checkpoint:
    ...
    def save_objects(self, models_to_save: Dict[str, Any], stage, current_stat,  **kwargs):
...
class ModelCheckpoint(object):
    ...
    def _initialize_model(self, model, weight_name):
    ...
    def save_best_models_under_current_metrics(
        self, model, metrics_holder: dict, **kwargs
    ):
        ...
        if stage == "train":
            ...
            if len(stats[stage]) > 0:
                ...
                for metric_name, current_metric_value in metrics.items():
                    ...
                    if metric_name == 'r2':
                        best_value = max(best_metric_from_stats, current_metric_value)
                    else:
                        best_value = min(best_metric_from_stats, current_metric_value)
        ...
        self._checkpoint.save_objects(models_to_save, stage, current_stat,  **kwargs)
